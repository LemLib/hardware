...
class Motor : public Encoder {
  public:
     ...
     * @code {.cpp}
     * void initialize() {
     *     // construct a new Motor object on port 1, which is reversed, and powers a mechanism that
     * spins at 15 rpm lemlib::Motor motor(-1, 15_rpm);
     * }
     * @endcode
     */
     ...
     * @endcode
     */
    int32_t isConnected() const;
    /**
     * @brief Get the relative angle measured by the motor
     *
     * The relative angle measured by the motor is the angle of the motor relative to the last time
     * the motor was reset. As such, it is unbounded.
     *
     * This function uses the following values of errno when an error state is reached:
     *
     * ENODEV: the port cannot be configured as a motor
     ...
     *
     * Setting the relative angle of the motor does so only on a software level, meaning that any
     * other lemlib::Motor objects will not register a change in the angle, only this object will
     *
     * This function uses the following values of errno when an error state is reached:
     *
     * ENODEV: the port cannot be configured as a motor
     *
     * This function sets the relative angle of the motor. The relative angle is the number of
     * rotations the motor has measured since the last reset. This function is non-blocking.
     *
     ...
     *         std::cout < "Relative angle: " << to_sDeg(motor.getAngle()) << std::endl; // outputs
     * 0 } else { std::cout << "Error setting relative angle!" << std::endl;
     *     }
     * }
     * @endcode
     */
     ...
     *     } else {
     *         std::cout << "Motor Temperature: " << units::to_celsius(motor.getTemperature()) <<
     * std::endl;
     *     }
     * }
     * @endcode
     ...
     *
     * @param outputVelocity the theoretical maximum output velocity of the motor, after gearing, to
     * set
